// Mostly uses code repurposed from https://github.com/kairusds/Hachimi-Edge/blob/main/src/windows/symbols_impl.rs
// For symbol remapping
...
// The Gallop namespace handles interactions with the client outside of msgpack response calls.
// Mostly related to il2cpp and such.
// Implements a 
namespace gallop 
{
    ...
    uintptr_t dlsym_gs(HMODULE handle, const char* name)
    {
        ...
        return reinterpret_cast<uintptr_t>(
            GetProcAddress(handle, it->second.c_str())
        );
    }
    ...
    void *push_dialog_orig = nullptr;
    void push_dialog_hook()
    {
        reinterpret_cast<decltype(push_dialog_hook)*>(push_dialog_orig)();
        spdlog::info("Hello from the push dialog hook!");
    }

    typedef bool (*is_debugger_attached_t)(void);
	...
	void StoryCharacter3D_LoadModel_hook(int charaId, int cardId, int clothId, int zekkenNumber, int headId, bool isWet, 
		bool isDirt, int mobId, int dressColorId, int charaDressColorSetId, void* zekkenName, int zekkenFontStyle, int color, int fontColor,
		int suitColor, bool isUseDressDataHeadModelSubId, bool useCircleShadow, int wetTexturePartsFlag) {

        spdlog::info("Hello from the StoryCharacter3D LoadModel hook!");
		...
		return reinterpret_cast<decltype(StoryCharacter3D_LoadModel_hook)*>(StoryCharacter3D_LoadModel_orig)(
			...
			suitColor, isUseDressDataHeadModelSubId, useCircleShadow, wetTexturePartsFlag);
	}
    ...
    void init()
    {
        ...
        // Test this shit by helloing
        auto dtm = umaimg->get_class("DialogTitleMenu", "Gallop");
        if (!dtm)
        {
            spdlog::error("[gallop] Aw! failed to get DialogTitleMenu from Gallop");
            return;
        }

        auto push_dialog = dtm->get_method("PushDialog");
        if (!push_dialog)
        {
            spdlog::error("[gallop] Aw! failed to get method PushDialog");
            return;
        }
        spdlog::info("[gallop] PushDialog: {}", std::to_string(reinterpret_cast<unsigned long long>(push_dialog)));

        MH_CreateHook(UFUNC(push_dialog), (void*)push_dialog_hook, &push_dialog_orig);
        MH_EnableHook(UFUNC(push_dialog));

        auto StoryCharacter3D = umaimg->get_class("StoryCharacter3D", "Gallop");
        if (!StoryCharacter3D)
        {
            spdlog::error("[gallop] Aw! failed to get StoryCharacter3D from Gallop");
            return;
        }

        auto LoadModel = StoryCharacter3D->get_method("LoadModel", 18);
        if (!LoadModel)
        {
            spdlog::error("[gallop] Aw! failed to get method LoadModel");
            return;
        }
        spdlog::info("[gallop] LoadModel: {}", std::to_string(reinterpret_cast<unsigned long long>(LoadModel)));

        MH_CreateHook(UFUNC(LoadModel), (void*)StoryCharacter3D_LoadModel_hook, &StoryCharacter3D_LoadModel_orig);
        MH_EnableHook(UFUNC(LoadModel));
    }
}