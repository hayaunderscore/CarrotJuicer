...
namespace Il2cpp
{
  ...
  // Takes mangled strings from a symbol map to remap into valid function names I think?
  inline uintptr_t dlsym_gs(HMODULE handle, const char* name, std::unordered_map<std::string, std::string> SYMBOL_MAP)
  {
    ...
    return reinterpret_cast<uintptr_t>(
      GetAddress(handle, it->second.c_str())
    );
  }
   ...
   */
  inline void initialize(std::unordered_map<std::string, std::string> SYMBOL_MAP)
  {
    if ( initialized )
      return;

    // Umamusume's GameAssembly.dll has mangled symbol names that UnityPlayer.dll has
    // In order to proceed further (without issues), we need to have a symbol map
    const auto GameAssemblyHandle = GetHandle( L"GameAssembly.dll" );

    // The symbol map is used here alongisde a call to a dlsym variant that will use the symbol map
#define DEF_ADDR( name, il2cpp_export, handle ) \
    name = reinterpret_cast<name##_t>( dlsym_gs(handle, il2cpp_export, SYMBOL_MAP) );
